...
def p_selectionStmt(p):
    ...
    if len(p) == 6:
        ...
        p[0] = AST.IfElseStmt(p[3], p[5], p[7])
...
def p_forIterationStmt(p):
    ...
    p[0] = AST.ForStmt(AST.Id(p[2]), p[4], p[5])
...
def p_expression(p):
    """expression : mutable '=' expression
                  ...
                  | simpleExpression"""
    if len(p) == 4:
        p[0] = AST.AssignmentStmt(p[2], p[1], p[3])
    else:
        p[0] = p[1]
...
def p_simpleExpression(p):
    """simpleExpression : arithExpression LTE arithExpression
                        ...
                        | arithExpression"""
    if len(p) == 4:
        p[0] = AST.RelBinExpr(p[2], p[1], p[3])
    else:
        p[0] = p[1]
...
def p_arithExpression(p):
    """arithExpression : arithExpression '+' arithExpression
                       ...
                       | value"""
    if len(p) == 4:
        p[0] = AST.ArithBinExpr(p[2], p[1], p[3])
    elif len(p) == 3:
...
def p_reference(p):
    """reference : ID '[' vectorItem ',' vectorItem ']'
                 | ID '[' vectorItem ']'"""
    if len(p) == 7:
        p[0] = AST.ReferenceStmt(AST.Id(p[1]), p[3], p[5])
    else:
        p[0] = AST.ReferenceStmt(AST.Id(p[1]), p[3])
